...
public class UFJI {
  ...
  private byte [] rank;
  // keep track of the largest element in the subtree rooted at p:
  private int [] largest;
  ...
  public void union (int p, int q) {
    ...
    if (rank[rootP] < rank[rootQ]) {
      ...
      largest[rootQ] = Math.max(largest[rootP],largest[rootQ]); // update the largest element under the new root
    }
    else if (rank[rootP] > rank[rootQ]) {
      parent[rootQ] = rootP;
      largest[rootP] = Math.max(largest[rootP],largest[rootQ]); // update the largest element under the new root
    }
  ...
  public static void main (String[] args) throws IOException { 
    ...
    for(int i=0; i<uf.getParentPointers().length; i++) System.out.print(uf.getParentPointers()[i] + " ");
    System.out.println();
